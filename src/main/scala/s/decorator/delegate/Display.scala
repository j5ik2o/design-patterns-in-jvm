...
object Display {
  ...
  def ofSideBorder(display: Display, ch: Char) = new Display(new SideBorderDelegate(display, ch))

  def ofFullBorder(display: Display) = new Display(new FullBorderDelegate(display))
}
